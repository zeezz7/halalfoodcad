...
import { IncomingForm } from "formidable";
import fs from "fs/promises"; // Import fs for handling file buffers
import dbConnect from "../../../../lib/mongodb";
import Rider from "../../../../models/Rider";
import { uploadToCloudinary } from "../../../../lib/cloudinary";

// Important: Disable bodyParser (for file uploads)
export const config = {
  api: {
    bodyParser: false,
    externalResolver: true, // Helps with Next.js 12+ deployments
  },
};
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Parse form data using formidable with buffer
    const form = new IncomingForm({
      multiples: true,
      keepExtensions: true,
    });
    ...
    // Handle file uploads with buffers
    if (files) {
      const documentTypes = [
        "governmentId",
        "driversLicense",
      ...
      for (const docType of documentTypes) {
        if (files[docType]) {
          ...
          if (file) {
            try {
              // Read file as buffer for Cloudinary upload
    ...
    return res
      ...
      .json({
        message: "Rider registration successful",
        rider: { id: newRider._id, email: newRider.email },
      });
  } catch (error) {
    console.error("Server error:", error);
    return res
